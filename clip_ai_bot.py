...
def wiki_summary(subject: str) -> Dict[str, str]:
    ...
    try:
        ...
        search = requests.get(search_url, params=search_params, timeout=8).json()
...
def build_queries(subject: str, fact: str, scene: str) -> Dict[str, List[str]]:
    ...
    related = []
    for q in [f"{subject} {scene}", scene, " ".join(tokens(fact, 6)), f"{scene} b roll", f"{scene} stock video"]:
        q = clean(q)
...
def subject_match(clip: Dict[str, Any], subject: str) -> bool:
    subtoks = tokens(subject, 8)
    if not subtoks:
        return False
    text = clean(" ".join(str(clip.get(k, "")) for k in ["title", "tags", "creator", "source", "pageUrl"])).lower()
    hits = sum(1 for t in subtoks if t in text)
    return hits >= (1 if len(subtoks) == 1 else min(2, len(subtoks)))


def scene_match(clip: Dict[str, Any], scene: str) -> bool:
    stoks = tokens(scene, 10)
    if not stoks:
        return False
    text = clean(" ".join(str(clip.get(k, "")) for k in ["title", "tags", "creator", "source", "pageUrl"])).lower()
    hits = sum(1 for t in stoks if t in text)
    return hits >= min(2, len(stoks))


def score_clip(clip: Dict[str, Any], subject: str, scene: str) -> float:
    text = clean(" ".join(str(clip.get(k, "")) for k in ["title", "tags", "creator", "source", "pageUrl", "license"])).lower()
    score = 0.0
    for t in tokens(subject, 8):
        if t in text:
            score += 15
    for t in tokens(scene, 10):
        if t in text:
            score += 4
    ...
    if clip.get("source") == "Wikimedia Commons":
        score += 6
    if "public" in clean(clip.get("license")).lower() or "cc0" in clean(clip.get("license")).lower():
        score += 4
    if clip.get("duration"):
        d = float(clip.get("duration") or 0)
        if 8 <= d <= 90:
            score += 3
    return score
        ...
        try:
            res = requests.get(
                ...
                params={
                    ...
                    "size": "medium",
                    "per_page": 80 if pages > 1 else 40,
                    "page": page,
                },
    ...
    for _ in range(pages):
        try:
            ...
            if not res.ok:
                break
            data = res.json()
            pages_data = list((data.get("query", {}).get("pages") or {}).values())
            for page in pages_data:
                ...
                title = clean(page.get("title") or "").replace("File:", ""))
                clips.append({
...
def strict_search(subject: str, fact: str, scene: str, pixabay_key: str, pexels_key: str, many: bool = False) -> Dict[str, Any]:
    qs = build_queries(subject, fact, scene)
    pages = 5 if many else 1
    raw: List[Dict[str, Any]] = []

    for q in qs["exact"]:
        raw.extend(search_pixabay(q, pixabay_key, pages))
        raw.extend(search_pexels(q, pexels_key, pages))
        raw.extend(search_commons(q, 2 if many else 1))
